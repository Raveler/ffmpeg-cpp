...
int main()
{
	...
	try
	{
		...
		auto muxer = std::make_unique<Muxer>("output.mpg");

		// Create a MPEG2 codec that will encode the raw data.
		auto codec = std::make_unique<VideoCodec>("mpeg2video");
		...
		auto videoFile = std::make_unique<RawVideoFileSource>("samples/carphone_qcif.y4m", encoder.get());
