...
int main()
{
	...
	try
	{
		...
		auto muxer = std::make_unique<ffmpegcpp::Muxer>("filtered_video.mp4");

		// Create a MPEG2 codec that will encode the raw data.
		auto codec = std::make_unique<ffmpegcpp::VideoCodec>(AV_CODEC_ID_MPEG2VIDEO);
		...
		auto filter = std::make_unique<ffmpegcpp::VideoFilter>("scale=640:150,transpose=cclock,vignette", encoder.get());

		// Load a video from a container and send it to the filter first.
		auto demuxer = std::make_unique<ffmpegcpp::Demuxer>("samples/big_buck_bunny.mp4");
